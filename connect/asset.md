# 沟通：数字资产

## 量潮工作档案

量潮工作档案已发布 0.0.1 版本
链接：GitHub - quanttide/quanttide-profile: 量潮工作档案
要点：
- 父仓库采用 Git 子模块结构管理多个独立文档模块
- domain/ 模块：领域知识模块（asset, innov, knowl, stdn, think, write）
- entity/ 模块：实体档案模块（company, founder）
- 新增 AGENTS.md：Agent 工作指南

### 量潮创始人档案

量潮创始人档案已发布 0.2.0 版本
链接：GitHub - quanttide/quanttide-profile-of-founder: 量潮创始人档案
要点：
- 本档案是我向公司提交海量碎片化想法的渠道。当前我正在验证个人提交流程的有效性，团队验证其作为人机协作资料的有效性。
- 本档案的 AGNETS 提供了一套 Vibe Coding人机协同的新范式。核心原则在于 AGNETS作为元认知，各级 index 提供陈述性记忆、REAMDE、ROADMAP、CHANGELOG 等提供程序性记忆。
- 本轮更新：大规模目录重构（think/acad、data、strategy 子目录迁移；asset、write 目录重组）

### 量潮科技档案

量潮科技档案已发布 0.1.0 版本
链接：GitHub - quanttide/quanttide-profile-of-business-entity: 量潮企业档案
要点：
- 新增 qtclass/：量潮课堂（培训品牌）
- 新增 qtdata/：量潮数据（数据品牌）
- 新增多个工作区模块（agent、brand、code、delib、enterpr、execute、hr、iam、learn、mr、product、scm）

### 领域档案

各领域子模块已发布 0.0.1 版本：
- domain/asset：数字资产
- domain/innov：创新管理
- domain/knowl：知识工程
- domain/stdn：标准化
- domain/think：认知工程
- domain/write：叙事工程
